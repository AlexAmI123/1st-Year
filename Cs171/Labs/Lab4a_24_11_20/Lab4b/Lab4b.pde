...
float by=-300;
float cx=-300; // Vector c resolved into components
float cy=-100;
Matrix a;
Matrix b;
Matrix c;
...
void draw()
{
 ...
 double length2 = c.norm2();
 Matrix c_unit = c.times(1/length);
 
 Matrix c_unit_T = c_unit.transpose();
 Matrix projection2 = c_unit_T.times(b);
 double lp2 = projection2.get(0,0);
 
 Matrix y = c_unit.times(lp2);
 // STEP 5. Insert code here to compute and draw the projection of b onto c
 
 float cx = (float)c.get(0,0);
 float cy = (float)c.get(1,0);
 float yx = (float)y.get(0,0);
 float yy = (float)y.get(1,0);
 
 stroke(0,0,0); // Use a black pen
 ellipse(X+yx,Y+yy,10,10); // point where b projects onto a
 line(X+yx,Y+yy,X+bx,Y+by); // line from a to point of projection on b
 stroke(0,0,255); // Make pen red
 arrow(X,Y,X+cx,Y+cy); // Draw vector a starting at (X,Y)
 stroke(0,255,0); // Make pen green
 arrow(X,Y,X+bx,Y+by); // Draw vector b starting at (X,Y)
 ...
 c.set(1,0,(double)mouseX-X);
 c.set(0,0,(double)mouseY-Y);
 }
 if (mouseButton == LEFT)
 {
